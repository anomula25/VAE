...
def generate_output(input_image, model_type):
    ...
    if model_type == 'VAE':
        resized_image = input_image.resize((128, 128))
    elif model_type == 'CNN':
        resized_image = input_image.resize((48, 48))
    else:
        raise ValueError("Invalid model type provided. Use 'VAE' or 'CNN'.")

    input_image_array = np.array(resized_image) / 255.0
    input_image_array = np.expand_dims(input_image_array, axis=0)  # Add batch dimension

    input_tensor = tf.convert_to_tensor(input_image_array, dtype=tf.float32)
    ...
    output_image = output['output_0'].numpy()  # Convert output to numpy array

    # Ensure the output is suitable for display
    output_image = np.squeeze(output_image)  # Remove singleton dimensions
    if output_image.ndim == 2:  # If grayscale, expand to 3 channels for consistent handling
...
# Streamlit interface for uploading and displaying images
